...
class MovieAnalyzer:
    DATA_DIR = os.path.abspath(".")  # ✅ Fixed DATA_DIR assignment
    ...
    def _load_movies(self):
        ...
        # 🔹 Extract genre IDs from dictionary format
        def extract_genre_ids(genre_dict_str):
            try:
                genre_dict = ast.literal_eval(genre_dict_str)  # Convert to dictionary
                return list(genre_dict.keys())  # Extract genre IDs
    ...
    def movie_type(self, n=10):
        ...
        genre_counts = pd.Series(
            [
                self.GENRE_MAPPING.get(genre, f"Unknown ({genre})")  # Handle missing genres
