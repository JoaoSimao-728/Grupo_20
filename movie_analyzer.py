...
class MovieAnalyzer:
    ...
    def _load_movies(self):
        ...
        # ✅ Define extract_genres inside _load_movies()
        ...
        # ✅ Fill missing summaries with a default message
    ...
    def releases(self):
        """Return movie releases per year."""
        self.movies_df["release_date"] = pd.to_numeric(self.movies_df["release_date"], errors="coerce")
        return self.movies_df.dropna(subset=["release_date"]).groupby("release_date").size().reset_index(name="count")
    ...
    def ages(self, mode="Y"):
        ...
        df = df[df["actor_dob"] > 1800]  # ✅ Filter incorrect birth years
    ...
    def actor_table(self, gender="All", min_height=1.5, max_height=2.0):
        ...
        return df[["actor_name", "actor_gender", "actor_height"]].head(10)  # ✅ Show top 10 results for preview
